 ...
 */
public class MapperConfigurationFactory implements BeanFactory<MapperConfiguration,MapperConfigurationContext> {

    List<AbstractMapperConfiguration> list = new ArrayList<>();
    ...
    public Set<MapperConfiguration> getObjects(MapperConfigurationContext data) {        
        ...
        for( AbstractMapperConfiguration c : list ){
            if( source.equals(c.getSourceType().getName()) && target.equals(c.getTargetType().getName()) ){
                contextList.add(c);
            }
        }
        return contextList;
